...
async function generateInterViewReportController(req, res) {
    try {
        ...
        if (req.file) {
            try {
                const pdfBuffer = Uint8Array.from(req.file.buffer)
                const pdfText = await (new pdfParse.PDFParse(pdfBuffer)).getText()
                resumeContent = pdfText.text
            } catch (pdfError) {
                return res.status(400).json({
                    message: "Failed to parse PDF. Please ensure the uploaded file is a valid PDF."
                })
            }
